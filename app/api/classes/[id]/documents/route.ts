...
export async function POST(req: NextRequest, { params }: Params) {
  try {
    ...
    const allowed = await can(session, "documents", "edit", { class_id: id })
    if (!allowed) return NextResponse.json({ error: "You don't have permission to upload documents for this class" }, { status: 403 })
...
export async function DELETE(req: NextRequest, { params }: Params) {
  try {
    ...
    const allowed = await can(session, "documents", "full", { class_id: id })
    if (!allowed) return NextResponse.json({ error: "You don't have permission to delete documents for this class" }, { status: 403 })
