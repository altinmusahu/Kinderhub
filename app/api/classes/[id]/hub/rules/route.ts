...
export async function GET(_req: NextRequest, { params }: Params) {
  try {
    ...
    const rules = await ClassRulesService.getByClassId(session.tenant_id, id)
...
export async function POST(req: NextRequest, { params }: Params) {
  try {
    ...
    if (!allowed) return NextResponse.json({ error: "You don't have permission to edit this class's rules" }, { status: 403 })

    const body = await req.json()
    const parsed = createClassRuleSchema.parse({ ...body, class_id: id, tenant_id: session.tenant_id })
