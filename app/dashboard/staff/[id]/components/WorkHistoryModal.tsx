...
function AddRecordForm({
  ...
  userId: string
  onAdded: (row: WorkTrackingRow) => void
}) {
  ...
  function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    const body = {
      position_name: fd.get("position_name"),
      start_date:    fd.get("start_date"),
      department_id: departmentId || null,
    }
    startSave(async () => {
      const res = await fetch(`/api/users/${userId}/work-tracking`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(body),
      })
      if (res.ok) {
        const newRow: WorkTrackingRow = await res.json()
        onAdded(newRow)
        setOpen(false)
        setDepartmentId("")
        setError("")
      ...
      <form onSubmit={handleSubmit}>
        <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
          <div>
            ...
            <input name="position_name" required placeholder="e.g. Lead Educator" style={fieldStyle} />
          </div>
          <div>
            <label style={labelStyle}>Start date *</label>
            <input name="start_date" type="date" required style={fieldStyle} />
          </div>
          <div>
            <label style={labelStyle}>Department</label>
            <DepartmentSelectPlain value={departmentId} onChange={setDepartmentId} style={fieldStyle} />
          </div>
        </div>
        ...
        <div style={{ display: "flex", gap: 8, marginTop: 14 }}>
          ...
          <button type="button" className="kh-btn" style={{ fontSize: 12.5 }} onClick={() => { setOpen(false); setDepartmentId(""); setError("") }}>
            Cancel
          </button>
        </div>
      </form>
...
export function WorkHistoryModal({ userId, onClose }: { userId: string; onClose: () => void }) {
  const [history, setHistory] = useState<WorkTrackingRow[] | null>(null)
  const [loading, setLoading] = useState(true)
  const [error, setError] = useState("")

  useEffect(() => {
    fetch(`/api/users/${userId}/work-tracking`)
      .then(r => r.json())
      .then(d => { setHistory(Array.isArray(d) ? d : []); setLoading(false) })
      .catch(() => { setError("Failed to load history."); setLoading(false) })
  }, [userId])

  function handleAdded(newRow: WorkTrackingRow) {
    setHistory(prev => {
      if (!prev) return [newRow]
      // mark the previous current record as closed (end_date = today) in UI
      return [newRow, ...prev.map(r => r.end_date === null ? { ...r, end_date: new Date().toISOString().slice(0, 10) } : r)]
    })
  }
    ...
    >
      ...
      >
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "20px 24px", borderBottom: "1px solid var(--kh-border)", flexShrink: 0 }}>
          <div>
            <div style={{ fontFamily: "var(--kh-font-serif)", fontSize: 20, color: "var(--kh-ink-900)" }}>Employment History</div>
            <div style={{ fontSize: 12, color: "var(--kh-ink-400)", marginTop: 2 }}>All work tracking records</div>
          </div>
          <button
            onClick={onClose}
        ...
        <div style={{ flex: 1, overflowY: "auto", padding: "20px 24px", display: "flex", flexDirection: "column", gap: 20 }}>
          ...
          {!loading && !error && history && history.length === 0 && (
            <div style={{ textAlign: "center", padding: 20 }}>
              <div style={{ fontSize: 13, color: "var(--kh-ink-400)" }}>No work tracking records yet.</div>
            </div>
          )}
          ...
          {!loading && !error && history && history.length > 0 && (
            <div style={{ position: "relative" }}>
              ...
              <div style={{ display: "flex", flexDirection: "column" }}>
                {history.map((wt, i) => {
                  const isCurrent = wt.end_date === null
                  return (
                    <div key={wt.id} style={{ display: "flex", gap: 18, paddingBottom: i < history.length - 1 ? 24 : 0 }}>
                      <div style={{ flexShrink: 0, width: 24, display: "flex", justifyContent: "center", paddingTop: 3 }}>
                        <div style={{
                          width: 12, height: 12, borderRadius: "50%",
                          background: isCurrent ? "var(--kh-peach)" : "var(--kh-border)",
                          border: isCurrent ? "2px solid #F0C4A8" : "2px solid var(--kh-border-soft)",
                          boxSizing: "border-box",
                          position: "relative", zIndex: 1,
                        }} />
                      </div>
                      ...
                      <div style={{ flex: 1, paddingBottom: 4 }}>
                        ...
                        {wt.department && (
                          <div style={{ fontSize: 12.5, color: "var(--kh-ink-500)", marginBottom: 4 }}>
                            {wt.department.name}
                          </div>
                        )}
                        <div style={{ fontSize: 11.5, color: "var(--kh-ink-400)", fontFamily: "var(--kh-font-mono)" }}>
                          {fmtDate(wt.start_date)} → {fmtDate(wt.end_date)}
                        </div>
                      </div>
                    </div>
                  )
