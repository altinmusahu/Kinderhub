...
export default function AddParentModal({ familyId, onClose, onSuccess }: Props) {
  const [form, setForm] = useState<FormState>({
    firstname: "", lastname: "", phone_number: "",
    personal_number: "", date_of_birth: "", address: "",
    pick_up: false,
  })
  ...
  async function handleSubmit(e: React.SyntheticEvent<HTMLFormElement>) {
    ...
    try {
      const res = await fetch("/api/parents", {
        ...
        body: JSON.stringify({ ...form, family_id: familyId }),
      })
      if (!res.ok) {
        const j = await res.json().catch(() => ({}))
        throw new Error(j.error ?? "Failed to add parent.")
      }
      onSuccess()
      onClose()
    } catch (err) {
    ...
    >
      <div style={{ background: "var(--kh-surface)", borderRadius: 18, width: "100%", maxWidth: 480, boxShadow: "0 24px 60px rgba(0,0,0,0.18)", display: "flex", flexDirection: "column", overflow: "hidden", maxHeight: "90dvh" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "18px 20px 14px", borderBottom: "1px solid var(--kh-ink-100)", flexShrink: 0 }}>
          <div>
            <div style={{ fontFamily: "var(--kh-font-serif)", fontSize: 20, color: "var(--kh-ink-900)" }}>Add parent / guardian</div>
