...
`RESOURCES` (14 keys: families, staff, classes, attendance, incidents, curriculum, hub, documents, food_supplies, billing, messages, calendar, settings, legal_entity) is the single source of truth for every resource key, label, and Zod validation — same "one file, imported everywhere" pattern as `lib/plans.ts`. `PermissionLevel = "none" | "view" | "edit" | "full" | "own_only"` — one level per cell, no compound/two-axis encoding. `own_only` implies edit-level access scoped to the caller's own records; exactly which actions that allows is resolved per-resource by the ownership checkers below, not stored as a second field.

> **`children` was retired as a separate resource** shortly after launch — the user removed it from both `RESOURCES` and the DB's `role_permissions` rows, reasoning that children/kids have no dedicated page or table of their own in this permission model; they're always accessed *through* a family (the Families list/detail pages, `AddKidButton`, kid-to-class assignment), so a `families` permission already governs them. Retiring it required removing the `children:` field from every role in `default-roles.ts` and deleting the (now-unused) `children` entry from `OWNERSHIP_CHECKERS` in `ownership.ts` — `isKidInOwnClass()` itself was kept, since `checkDocumentSubject()` (for documents attached to a specific kid) still calls it. No route ever used `"children"` as a `can()` resource argument — the kid-to-class assignment route (`kids/[class_id]/kids/[kid_id]`) already used `"classes"`, so nothing else needed to change.
...
- **Live workspace-item badges**: `WORKSPACE_ITEMS`' `badge` field is now `number | "families" | "staff" | "classes" | null` — a string key means "resolve this from `navCounts` at render time" via `resolveBadge()`, a literal number keeps the old hardcoded value. `app/dashboard/layout.tsx` runs three `select("id", { count: "exact", head: true })` queries (Families/Staff scoped by `tenant_id`; Classes **not** tenant-scoped, matching `ClassesRepository.findAll()`'s existing lack of tenant filtering elsewhere) alongside its existing session/permissions fetch, and passes the result down as a `navCounts` prop through `DashboardShell` → `Sidebar`. **Billing (`9`) and Messages (`3`) badges stay hardcoded** — both pages are still 100% mock data (see Pages Summary), so there's nothing real to count yet.
...
- **Not gated**: `/dashboard/users` — an unlinked legacy prototype page (uses the old hardcoded-tenant `lib/db.ts` helper, not `UserService`) discovered while auditing every route under `app/dashboard/`. **Deleted** by the user shortly after this was flagged — no longer exists.

### Row/action-level UI gating on top of page guards
Page guards (`hasAnyAccess`) answer "can this role see the page at all" — they don't distinguish *within* a page between read-only (`"view"`) and actionable (`"edit"`/`"full"`) access. `lib/permissions/can.ts` gained `getMyPermissionLevel(session, resource)` (exposes the raw `PermissionLevel`, not just a boolean) specifically for pages that need to hide row-level actions for view-only roles while still showing the data itself.

**`app/dashboard/families/page.tsx`** is the first page built this way, after the user set Assistant's `families` level to `"view"` via the Roles & Permissions UI and found the "Open →" link and "+ Add family" button still fully active for that role — page guards alone don't catch this, since `"view"` still passes `hasAnyAccess()`.
- `columns` was converted from a static module-level array to `buildColumns(canDrillDown: boolean)`, since the family-name cell and the "Open →" action cell both need to conditionally render a `<Link>` vs. plain non-interactive markup based on the caller's level.
- `canDrillDown = level !== "view"` — hides both the family-name link and the whole trailing "actions" column (rather than showing a disabled/dash state) when `false`.
- `canCreate = level === "edit" || level === "full"` — hides the "+ Add family" button entirely for `"view"` and `"own_only"` (a custom role could set `own_only` on `families`, which per `can()`'s own semantics allows edit-level actions on *owned* records but not indiscriminate creation of new ones — `canCreate` deliberately excludes `own_only` here for that reason).
- This same pattern (`getMyPermissionLevel()` → derive a couple of booleans → thread them into column-builder functions and button visibility) is the template to reuse on other list pages (Staff, Classes, Documents, etc.) if the same "view-only role can still click Open/Add" gap is found there — it hasn't been rolled out beyond Families yet.

### Detail-page guard + in-page edit gating (`app/dashboard/families/[id]/page.tsx`)
Fixing the list page's "Open →" link doesn't stop someone who already knows or guesses a family's id from hitting `/dashboard/families/[id]` directly — the detail page itself had no permission check of its own. Unlike the page-level `hasAnyAccess()` guards used elsewhere (plain "can this role see the page," ignoring which specific record), this page uses the real `can()` check against the specific family id, since `own_only` needs a concrete record to resolve ownership against and a coarser `hasAnyAccess()` would incorrectly let an `own_only` role view *any* family's page, not just their own:
- `const canView = await can(session, "families", "view", id); if (!canView) return <AccessDenied />` — blocks entirely for `"none"`, and for `own_only` roles defers to `isFamilyInOwnClass()` (true only if one of that family's kids is in a class the caller leads/assists).
- `const canEdit = await can(session, "families", "edit", id)` — passed down into `FamilyTabs` as a new `canEdit` prop, threaded further into `OverviewTab` → `ChildrenCard`'s `AddKidButton` and the Parents tab's `showButton` (which already existed as a prop, previously driven by which tab was active rather than permissions — now driven by both).
- **`DocumentsTab.tsx`'s upload/delete gating** — this component is shared between the Family and Staff detail pages, so it gained a `canEdit?: boolean` prop (defaults to `true`, to avoid silently breaking any caller not yet updated) rather than each caller re-implementing the same hide-the-button logic. When `false`, both the "+ Upload document" control and each row's "Delete" button are omitted entirely (not just disabled). `families/[id]/FamilyTabs.tsx` passes its existing `canEdit`; `staff/[id]/EmployeeTabs.tsx` needed a new `canEdit` prop added and threaded down from `staff/[id]/page.tsx`, which — like the Families detail page before it — had **no permission check of its own at all** until this pass added `can(session, "staff", "view"/"edit", id)`.

### `own_only` list-page filtering (`app/dashboard/classes/page.tsx`, `classes/[id]/page.tsx`)
Prompted by the user directly testing the Assistant role and correctly identifying that an `own_only` level on `classes` needs the *list itself* filtered server-side — not just gated at the page level — since `ClassWithRelations` already carries `lead_user_id`/`assistant_user_id` on every row.
- **List page**: `getMyPermissionLevel(session, "classes")` resolves the caller's level; when it's `"own_only"`, `classes` is filtered in-memory to rows where `lead_user_id === session.sub || assistant_user_id === session.sub` before any of the page's stat totals (`totalEnrolled`, `totalWaitlist`) are computed from it — so the KPI numbers reflect only the classes the caller can actually see, not the tenant-wide totals. `canCreate = level === "full"` gates the "+ Add class" button — deliberately **not** `"edit"`, since creating a brand-new class has no existing record for `own_only` to scope against (mirrors the `POST /api/classes` route's own `can(session, "classes", "full")` check with no `ownerCheckId`).
- **Detail page**: had no permission check at all before this (relied purely on middleware, same gap `families/[id]` and `staff/[id]` had). Added `can(session, "classes", "view", id)` right after the class is fetched — for `own_only`, this resolves through `isClassLeadOrAssistant()` against the specific class id, so a caller can't open a class-by-URL that isn't theirs even if the list page already filtered it out of view. Required restructuring the page's auth block slightly: `session` previously only existed inside the initial `try/catch` (only `tenant_id` was extracted out), and had to be hoisted to an outer `let` so the permission check could use it afterward.
...
| `LeavesTab` | GET `/api/users/[id]/leaves` (own history + summary) | POST (request leave) / DELETE (cancel own pending) / PATCH (edit balance — gated, see below) |
...
### Leaves / Vacation Balance System (`leave_requests` module, `EmployeeTabs → LeavesTab`, `staff/page.tsx → StaffLeavesAdminTab`)

Built on top of two tables the user created directly in Supabase (`leave_requests`, `leave_balances` — see `database.md`), not via a migration file in `supabase/migrations/`. Reuses the existing `"staff"` permission resource — no new `ResourceKey` was added.

- **Module** (`app/api/modules/leave_requests/`): standard types/repository/service/validation split.
  - `LeaveRequestsService.create()` computes `total_days` as **weekdays only** (weekends excluded), matching the schema's own column comment — not calendar-inclusive days.
  - `LeaveRequestsService.getSummaryForUser()` / `setBalance()` derive Entitled/Used/Pending/Remaining from `leave_requests` (this year's approved/pending sums) plus a `leave_balances` row, which is **lazily get-or-created** on first read (`LeaveBalancesRepository.getOrCreateAnnual`) with `DEFAULT_ANNUAL_ENTITLEMENT_DAYS = 22` if none exists yet for the current year.
- **API routes**: `GET/POST/PATCH /api/users/[id]/leaves` (list+summary / create / set balance), `PATCH/DELETE /api/users/[id]/leaves/[leaveId]` (approve-or-reject / cancel), `GET /api/leaves` (all-staff admin view + tenant summary).
  - Reviewing (`PATCH .../[leaveId]`) and setting a balance (`PATCH .../leaves`) both use `getMyPermissionLevel(session, "staff")` directly and require `edit`/`full` — **not** `can(session, "staff", "edit")` with no `ownerCheckId`, which throws for `own_only` callers (`can()`'s `own_only` branch requires an `ownerCheckId` to resolve ownership against; there's no single "owner" for a review/balance action). This was a real bug hit during development — both routes originally called `can()` bare and 500'd for `own_only` sessions instead of cleanly returning 403.
- **Employee profile → Leaves tab** (`LeavesTab.tsx`): 4 stat tiles (Entitled/Used/Pending/Remaining), a leave history table, and a "Request leave" modal (uses the shared `Modal`/`MField`/`MInput`/`MSelect` primitives from `components/ui/Modal.tsx`).
- **Staff directory → Leaves tab** (`StaffLeavesAdminTab.tsx`, gated to `edit`/`full` staff level — hidden entirely, not just disabled, for `own_only`): tenant-wide stat tiles, search/type/status filters, and inline Approve/Reject on pending rows.
- **"Edit balance" hardcoded role block**: separately from the `edit`/`full` permission gate above, the Edit-balance button/panel (in both `LeavesTab.tsx` and `StaffLeavesAdminTab.tsx`) is hidden for any session whose `role` (plain JWT claim, not permission level) is exactly `"Lead Teacher"`, `"Staff"`, or `"Assistant"` — a literal string blocklist (`ROLES_BLOCKED_FROM_EDITING_BALANCE`), per explicit user instruction to not derive this from the permission-level system at all. The underlying API route (`PATCH /api/users/[id]/leaves`) still independently requires `edit`/`full` regardless of this UI-only list.
- **`AddStaffModal.tsx`** gained an "Annual leave days" field (default `22`) that seeds the new employee's `leave_balances` row at creation time via `LeaveBalancesRepository.create()` — currently unreachable end-to-end because `POST /api/users` is broken (see Known Issues, `users.role`).
- **Dashboard Overview "My leave balance" card** (`app/dashboard/page.tsx`) is a **static mock** stat card (`{ daysLeft: 12, entitled: 20, used: 5 }`), consistent with the rest of that page (see Pages Summary) — not wired to the real `leave_requests`/`leave_balances` tables. `kh-stats-grid--5` modifier class added to `globals.css` to fit 5 stat cards on that one page without affecting Billing/Documents, which also use the base `kh-stats-grid` class at 4 columns.
...
21. **`users.role` (free text) has since been dropped from the live schema — code has not caught up.** The Roles & Permissions system originally only added the nullable `role_id` FK column alongside the old `role` text column (see below), intentionally leaving `role` in place. At some point after that, `role` was dropped from the actual `users` table (confirmed live: `select role from users` → `column users.role does not exist`), but `UserRepository.createWithId()` and `user.types.ts`' `User` type still reference it. **This currently breaks `POST /api/users` entirely** — creating a new staff member fails with `Could not find the 'role' column of 'users' in the schema cache`, discovered while testing the new "Annual leave days" field on `AddStaffModal.tsx`. Not yet fixed — flagged, not addressed, since it's outside the scope of the leaves feature that surfaced it. Note `SessionPayload.role` (the JWT claim, used for the leave-balance role blocklist and elsewhere) is a *separate* thing — it's minted at login time from whatever `role` string existed then, not read from this column per-request, so it's unaffected by the column's removal.
...
24. **The admin Leaves tab's per-employee "Leave balances" panel can only ever seed a balance one employee at a time, on demand** — `EmployeeBalanceRow` in `StaffLeavesAdminTab.tsx` fetches each employee's summary via `GET /api/users/[id]/leaves` when the panel is expanded, which lazily get-or-creates their `leave_balances` row as a side effect of that read. There's no bulk "set default entitlement for everyone" action; onboarding an already-existing tenant's employees onto the leaves system means opening this panel (or their own profile Leaves tab) once per person.
25. **`classes` count in the sidebar's live "Classes" badge is not tenant-scoped** — inherited from `ClassesRepository.findAll()`, which has never filtered by `tenant_id` (see the class detail page notes above). In a single-tenant dev environment this is invisible; in a real multi-tenant deployment the Classes badge would overcount, showing every tenant's classes combined.
